...
router.get('/portfolio/history', requireDemoMode, apiAuth, async (req, res) => {
  try {
    const clerkId = (req as any).auth?.userId;
    if (!clerkId) {
      return res.status(401).json({ success: false, error: 'Unauthorized' });
    ...
    const lendingPositions = await prisma.demoLendingPosition.findMany({
      where: { userId: user.id },
    });

    const currentPropertyValue = demoHoldings.reduce((sum, h) => {
      const mockProp = MOCK_PROPERTIES[h.propertyId];
      const price = mockProp?.tokenPrice || Number(h.averageCost);
      return sum + (h.quantity * price);
    }, 0);

    const totalInvested = demoHoldings.reduce((sum, h) => sum + Number(h.totalInvested), 0);
    const totalLendingDeposits = lendingPositions.reduce((sum, p) => sum + Number(p.depositedAmount), 0);

    const walletBalance = user.vaultAccount
      ? Number(user.vaultAccount.demoUsdcBalance) + Number(user.vaultAccount.demoBtcBalance) + Number(user.vaultAccount.demoParcoBalance)
      : 13000;

    const currentTotal = currentPropertyValue + walletBalance + totalLendingDeposits;
    const startingBalance = 13000;

    const chartData: Array<{ name: string; value: number; date: string }> = [];
    const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const now = new Date();
    
    const valueEvents: Array<{ date: Date; totalValue: number }> = [];
    let runningTotal = startingBalance;
    
    const accountCreatedAt = user.createdAt || new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    valueEvents.push({ date: accountCreatedAt, totalValue: startingBalance });
    
    transactions.forEach(tx => {
      const txDate = tx.completedAt || tx.createdAt;
      const amount = Number(tx.amount) || 0;
      
      const addToTotal = ['DEPOSIT', 'RENT_DISTRIBUTION', 'BORROW', 'INTEREST'];
      const subtractFromTotal = ['WITHDRAWAL', 'REPAY', 'FEE'];
      
      if (addToTotal.includes(tx.type)) {
        runningTotal += amount;
        valueEvents.push({ date: txDate, totalValue: runningTotal });
      } else if (subtractFromTotal.includes(tx.type)) {
        runningTotal -= amount;
        valueEvents.push({ date: txDate, totalValue: runningTotal });
      }
    });
    ...
    for (let i = 5; i >= 0; i--) {
      if (i === 0) {
        ...
        const bucketDate = new Date(now.getFullYear(), now.getMonth() - i + 1, 0, 23, 59, 59, 999);
        
        const relevantEvents = valueEvents
          .filter(e => e.date <= bucketDate)
          .sort((a, b) => b.date.getTime() - a.date.getTime());
        
        const value = relevantEvents.length > 0 ? relevantEvents[0].totalValue : startingBalance;
        
        chartData.push({
          name: monthNames[bucketDate.getMonth()],
