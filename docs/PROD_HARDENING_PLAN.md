...
| C3 | **Uploads** | Local disk storage | `multer.diskStorage` writes to `attached_assets/uploads/`. On Replit's ephemeral filesystem, files are lost on redeploy. No virus scanning. | Migrate to R2/S3 with signed URLs. Add file type validation and virus scanning. |
| C4 | **Database** | Migrations abandoned | An initial migration exists (`20251201202040_init`) but subsequent schema changes use `prisma db push --accept-data-loss` instead of `prisma migrate`. Migration history is out of sync with the live schema. | Reconcile migration history with current schema. Use `prisma migrate dev` / `prisma migrate deploy` for all future changes. |
| C5 | **Jobs** | In-process cron | `node-cron` runs inside the Express process. If the server crashes or restarts mid-job, work is lost. No job deduplication across replicas. | Move to external queue (BullMQ + Redis, or cloud scheduler). Add idempotency keys. |
| C6 | **Logging** | No structured logging | All logging is `console.log`/`console.error`. No log levels, no correlation IDs, no request tracing, no log aggregation. | Adopt Pino or Winston with JSON output. Add request ID middleware. Ship to Datadog/Axiom. |
