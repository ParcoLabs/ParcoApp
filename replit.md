...
- **Document Requirements**: `GET /api/issuance/case/:caseId/requirements` returns `requiredDocTypes` (from IssuanceTemplate rules), `uploadedDocTypes` (from IssuanceDocument), `missingDocTypes` (computed diff), and `criticalKeys`. TokenizerPreDashboard shows "Missing Documents" alert with upload CTAs linked to existing file inputs. AdminTokenizations shows required vs present doc types with status indicators in the detail drawer.
- **Extraction Quality Scoring**: `server/services/extractionQuality.ts` — `computeExtractionQuality(caseId)` scores extraction quality (0-100) based on critical field coverage and confidence. Missing critical keys penalize score; confidence < 0.65 triggers NEEDS_REVIEW. Sets `extractionScore` and `extractionQualityStatus` (PASS|NEEDS_REVIEW|FAIL|PENDING) on IssuanceCase. Called automatically after extraction completes. Status transition to REVIEW_READY gates on both `eligibilityStatus` and `extractionQualityStatus` being PASS; admin override with reason creates EXTRACTION_OVERRIDE audit event. AdminTokenizations shows score and quality badge.
